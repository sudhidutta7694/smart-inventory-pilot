...
const EastDashboard = () => {
  ...
  // Auto-switch to East user if available
  useEffect(() => {
    const eastUser = localStorage.getItem('user_east');
    if (eastUser && (!user || user.warehouse !== 'East')) {
      try {
        const parsedUser = JSON.parse(eastUser);
        localStorage.setItem('currentUser', eastUser);
        window.location.reload(); // Refresh to apply auth change
      } catch (error) {
        console.error('Error loading East user:', error);
      }
    }
  }, [user]);
        ...
        >
          <div className="flex h-14 items-center justify-between px-6">
            <div className="flex items-center space-x-4">
              <h1 className="text-lg font-semibold">East Warehouse Dashboard</h1>
              {user && (
